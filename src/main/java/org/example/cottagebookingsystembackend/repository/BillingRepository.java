...
public interface BillingRepository extends CrudRepository<Billing, Long> {
    @Query("SELECT r, s, b FROM ServicesOfReservation sor JOIN Reservation r ON r.reservationId = sor.reservationId JOIN ServiceModel s ON s.serviceId = sor.serviceId JOIN Billing b ON b.reservationId = r.reservationId WHERE r.confirmationDate<:confirmationDate")
